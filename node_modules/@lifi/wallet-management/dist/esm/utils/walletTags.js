...
export const getConnectorTagType = (connectorId, isConnected) => {
    if (isConnected) {
...
export const getWalletTagType = (wallet, connectedConnectorIds) => {
    let walletTagType;
    if (wallet.connectors.length > 1) {
        walletTagType = wallet.connectors.some((connector) => {
            ...
            return (connectorId &&
                getConnectorTagType(connectorId, connectedConnectorIds.includes(connectorId)) === WalletTagType.Connected);
        })
            ? WalletTagType.Connected
            : WalletTagType.Multichain;
    }
    else if (wallet.connectors.length === 1) {
        const connectorId = getConnectorId(wallet.connectors[0].connector, wallet.connectors[0].chainType);
...
//# sourceMappingURL=walletTags.js.map