 ...
 */ const getWalletAccounts = (client)=>{
   ...
   */ const verifiedCredentials = (_user_verifiedCredentials = user == null ? void 0 : user.verifiedCredentials) != null ? _user_verifiedCredentials : [];
    verifiedCredentials.filter((verified)=>verified.format === JwtVerifiedCredentialFormatEnum.Blockchain).map(convertVerifiedCredentialToWalletAccount).forEach((veririedWalletAccount)=>walletAccountsMap.set(veririedWalletAccount.id, veririedWalletAccount));
    return Array.from(walletAccountsMap.values()).map((walletAccount)=>assignClient(walletAccount, client));
};
...
class InvalidRedirectStorageStateError extends BaseError {
    constructor(){
        super({
            cause: null,
...
class MissingRedirectStorageStateError extends BaseError {
    constructor(){
        super({
            cause: null,
...
class MissingSocialUrlParamError extends BaseError {
    constructor(param){
        super({
            cause: null,
...
class NoAddressFoundError extends BaseError {
    constructor(){
        super({
            cause: null,
...
class WalletAccountAlreadyVerifiedError extends BaseError {
    constructor(address){
        super({
            cause: null,
...
/** Redirects the user to the OAuth provider's authorization page */ const signInWithSocialRedirect = async (/** The DynamicClient instance. */ client, /** The options for the sign in with social redirect. */ { provider, redirectUrl, captchaToken })=>{
    ...
    await createApiClient(client).initAuth({
        environmentId: core.environmentId,
 ...
 */ const deleteMfaDevice = async (device, { mfaAuthToken })=>{
    ...
    const core = getCore(client);
    const apiClient = createApiClient(client);
    assertDefined(device.id, 'device.id is required');
...
const assertWalletAccountSigningAvailability = async (walletAccount)=>{
    const walletProvider = getWalletProviderFromWalletAccount(walletAccount);
 ...
 */ const connectWithWalletProvider = async (walletProvider)=>{
    const client = getClient(walletProvider);
   ...
   */ if (currentAddresses.includes(selectedAddress)) {
    ...
    return assignClient(walletAccount, client);
};
 ...
 */ const selectPrimaryWalletAccount = async (walletAccount)=>{
    ...
    if (walletAccount.verifiedCredentialId) {
        ...
        setUnverifiedWalletAccount(client, _extends({}, walletAccount, {
